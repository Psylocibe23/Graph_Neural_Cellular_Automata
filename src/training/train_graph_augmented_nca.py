...
def main():
    ...
    base_dir = os.path.join("outputs", "graphaug_nca", "train_inter_loss", active_target_name)
    ...
    nca_param_count = count_parameters(model) + count_parameters(graph_aug)
    ...
    if ckpt_files:
        ...
        # restore graph augmentation if present
        if "graph_aug_state" in checkpoint:
    ...
    for epoch in trange(start_epoch, total_epochs + 1, desc="Epochs"):
        ...
        for step in trange(steps_per_epoch, desc=f"Epoch {epoch}", leave=False):
            ...
            for t in range(max_steps):
                ...
                if mask.any():
                    # --- Graph augmentation ---
                    graph_message = graph_aug(state[mask])
                    # Add message to state before NCA update
                    state[mask] = model(state[mask] + graph_message, fire_rate=fire_rate)
            ...
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(list(model.parameters()) + list(graph_aug.parameters()), 0.5)
            optimizer.step()
        ...
        # Save checkpoint (including graph_aug state!)
